...
export class AppState {
  ...
  @Selector() static selectUnit(state: AppStateModel): string {
    return state.unit;
  }

  @Selector() static selectCitiesWeatherInfo(
    state: AppStateModel
  ): WeatherByCityName[] {
    return state.citiesWeatherInfo;
  }

  @Selector() static selectDetailOfCity(
    state: AppStateModel
  ): WeatherDetailOfCity {
    return state.detailOfCity!;
  }

  @Selector() static selectIsLoading(state: AppStateModel): Boolean {
    return state.httpRequests.length > 0;
  }
  ...
  getCitiesWeatherInfo(
    ...
    zip(
      ...action.cities.map((city) =>
        this.weatherService.getWeatherByCityName(city, unit)
      )
    ).subscribe((res) => ctx.patchState({ citiesWeatherInfo: res }));
  }
  ...
  getDetailOfCity(ctx: StateContext<AppStateModel>, action: GetDetailOfCity) {
    let unit = ctx.getState().unit;
    this.weatherService
      .getWeatherDetailOfCity(action.lat, action.lon, unit)
      .subscribe((res) =>
        ctx.patchState({ detailOfCity: { ...res, name: action.cityName } })
      );
  }
  ...
  recordRequest(ctx: StateContext<AppStateModel>, action: RecordRequest) {
    ctx.patchState({
      httpRequests: [...ctx.getState().httpRequests, action.request],
    });
  }
